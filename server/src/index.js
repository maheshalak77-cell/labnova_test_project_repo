...
//   if (!origin) return callback(null, true); // same-origin / curl / server-to-server
//   if (explicitOrigins.includes(origin)) return callback(null, true);
//   if (NODE_ENV !== "production" && localhostPattern.test(origin))
...
//     console.error("\n⚠️  Could not connect to MySQL / create schema.");
//     console.error(
//       "   Check the DB_* values in server/.env — see server/.env.example.",
//     );
//     console.error("   Error:", err.message, "\n");
//     console.error(
//       "   The server will still start, but every API call that touches the database will fail until this is fixed.\n",
//     );
//   }
...
//     console.log(`LabNova API listening on http://localhost:${PORT}`);
//     console.log(
//       `Allowing requests from: ${explicitOrigins.join(", ")}${NODE_ENV !== "production" ? " (+ any http://localhost:<port> in dev)" : ""}`,
//     );
//   });
// }
...
// "https://labnova.vercel.app,https://www.labnova.com.au"
//
// Below that we ALSO always allow the site's known production domains, even
// if someone forgets to add one to the CLIENT_ORIGIN env var on Railway.
// This is what was causing the "Failed to fetch" / CORS 403 errors on
// https://www.labnovascientific.com.au and https://www.labnova.com.au —
// those two domains were never added to CLIENT_ORIGIN in Railway.
const DEFAULT_PRODUCTION_ORIGINS = [
  "https://labnovascientific.com.au",
  "https://www.labnovascientific.com.au",
  "https://labnova.com.au",
  "https://www.labnova.com.au",
];
...
// Also always trust any Vercel-issued URL for this app (production alias,
// preview deployments, etc.) so redeploys / new preview links never need a
// manual CORS update.
