...
//   try {
//     const [result] = await pool.query("INSERT INTO categories (name, slug, sort_order) VALUES (?, ?, ?)", [
//       name.trim(),
//       slug,
//       Number(sortOrder) || 0,
//     ]);
...
//   if (!name || !name.trim()) return res.status(400).json({ error: "Category name is required." });
//   const slug = slugify(name);
//   await pool.query("UPDATE categories SET name = ?, slug = ?, sort_order = ? WHERE id = ?", [
//     name.trim(),
//     slug,
//     Number(sortOrder) || 0,
//     req.params.id,
...
//   if (count > 0) {
//     return res.status(409).json({ error: `Cannot delete: ${count} product(s) still use this category. Move or delete them first.` });
//   }
...
//     isActive: body.isActive === undefined ? true : body.isActive === "false" ? false : !!body.isActive,
//   };
// }
...
//   if (!data.categoryId) return res.status(400).json({ error: "Category is required." });
//   if (!data.sku) return res.status(400).json({ error: "SKU is required." });

//   const image = req.file ? `/uploads/${req.file.filename}` : req.body.imageUrl || "/assets/products/placeholder.png";
...
//   } catch (err) {
//     if (err.code === "ER_DUP_ENTRY") return res.status(409).json({ error: "A product with this name or SKU already exists." });
//     throw err;
//   }
// });
...
//   const slug = slugify(data.name);
//   const setImage = req.file ? `/uploads/${req.file.filename}` : req.body.imageUrl;
...
//       total: (itemsByQuote[q.id] || []).reduce((sum, i) => sum + i.lineTotal, 0),
//     }))
//   );
// });
...
//   const [[{ totalQuotes }]] = await pool.query("SELECT COUNT(*) AS totalQuotes FROM quotes");
//   const [[{ totalProducts }]] = await pool.query("SELECT COUNT(*) AS totalProducts FROM products");
//   const [[{ totalCategories }]] = await pool.query("SELECT COUNT(*) AS totalCategories FROM categories");
//   const [[{ newQuotes }]] = await pool.query("SELECT COUNT(*) AS newQuotes FROM quotes WHERE status = 'new'");
...
//   const [statusRows] = await pool.query(`SELECT status, COUNT(*) AS count FROM quotes GROUP BY status`);
...
//     quotesByStatus: statusRows.map((r) => ({ status: r.status, count: r.count })),
//     topProducts: topProductRows.map((r) => ({ name: r.product_name, units: Number(r.units) })),
//     productsByCategory: categoryRows.map((r) => ({ name: r.name, count: r.count })),
//   });
// });
