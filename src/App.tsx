...
//   const cartCount = useMemo(() => cart.reduce((sum, line) => sum + Object.values(line.quantities).reduce((a, b) => a + b, 0), 0), [cart]);
//   const openProduct = (p: Product) => navigate(`/products/${p.id}`);

//   const addToCart = (quantities: Record<string, number>) => {
//     if (!modalProduct) return;
//     const clean = Object.fromEntries(Object.entries(quantities).filter(([, qty]) => qty > 0));
//     setCart((current) => {
//       const found = current.find((x) => x.productId === modalProduct.id);
//       if (!found) return [...current, { productId: modalProduct.id, quantities: clean }];
//       return current.map((x) =>
//         x.productId === modalProduct.id
//           ? { ...x, quantities: Object.fromEntries(Object.entries(clean).map(([size, qty]) => [size, (x.quantities[size] || 0) + qty])) }
//           : x
//       );
//     });
...
//     setCart((current) =>
//       current
//         .map((x) => (x.productId === id ? { ...x, quantities: { ...x.quantities, [size]: qty } } : x))
...
//       return <AdminLogin onLogin={admin.login} />;
//     }
//     const renderAdminPage = () => {
//       if (route === "/admin/quotes") return <AdminQuotes token={admin.token!} />;
//       if (route === "/admin/products") return <AdminProducts token={admin.token!} />;
//       if (route === "/admin/categories") return <AdminCategories token={admin.token!} />;
...
//   const renderPage = () => {
//     if (route === "/") {
//       return <Home navigate={navigate} openProduct={openProduct} addProduct={setModalProduct} products={catalog.products} loading={catalog.loading} error={catalog.error} />;
//     }
//     if (route === "/products") {
//       return (
//         <ProductsPage
...
//       return <ProductDetail slug={slug} navigate={navigate} addProduct={setModalProduct} />;
//     }
//     if (route === "/about") return <AboutPage navigate={navigate} />;
//     if (route === "/contact") return <ContactPage navigate={navigate} cart={cart} clearCart={() => setCart([])} products={catalog.products} />;
//     if (route === "/cart") return <CartPage navigate={navigate} cart={cart} updateLine={updateLine} removeLine={(id) => setCart((c) => c.filter((x) => x.productId !== id))} products={catalog.products} />;
//     return <Home navigate={navigate} openProduct={openProduct} addProduct={setModalProduct} products={catalog.products} loading={catalog.loading} error={catalog.error} />;
//   };
...
//       <div className="page-transition" key={route}>{renderPage()}</div>
//       <Footer navigate={navigate} openTerms={() => setTerms(true)} />
//       {modalProduct && <AddModal product={modalProduct} onClose={() => setModalProduct(null)} onConfirm={addToCart} />}
//       {terms && <TermsModal onClose={() => setTerms(false)} />}
//       <div className={`toast ${toast ? "toast--show" : ""}`}><Icon name="check" size={18} />{toast}</div>
//     </>
//   );
// }
