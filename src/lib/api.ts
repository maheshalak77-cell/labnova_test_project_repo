...
// //     request<Product[]>(`/products${categorySlug && categorySlug !== "all" ? `?category=${encodeURIComponent(categorySlug)}` : ""}`),
...
// //   }) => request<{ id: number; emailSent: boolean }>("/quotes", { method: "POST", body: JSON.stringify(payload) }),
// // };
...
// //     request<{ token: string }>("/admin/login", { method: "POST", body: JSON.stringify({ username, password }) }),

// //   verify: (token: string) => request<{ role: string }>("/admin/me", { headers: authHeaders(token) }),

// //   getStats: (token: string) => request<AdminStats>("/admin/stats", { headers: authHeaders(token) }),

// //   getQuotes: (token: string) => request<AdminQuote[]>("/admin/quotes", { headers: authHeaders(token) }),
// //   updateQuoteStatus: (token: string, id: number, status: string) =>
// //     request(`/admin/quotes/${id}/status`, { method: "PUT", headers: authHeaders(token), body: JSON.stringify({ status }) }),

// //   getCategories: (token: string) => request<Category[]>("/admin/categories", { headers: authHeaders(token) }),
// //   createCategory: (token: string, name: string) =>
// //     request<Category>("/admin/categories", { method: "POST", headers: authHeaders(token), body: JSON.stringify({ name }) }),
// //   updateCategory: (token: string, id: number, name: string) =>
// //     request<Category>(`/admin/categories/${id}`, { method: "PUT", headers: authHeaders(token), body: JSON.stringify({ name }) }),
// //   deleteCategory: (token: string, id: number) =>
// //     request<void>(`/admin/categories/${id}`, { method: "DELETE", headers: authHeaders(token) }),

// //   getProducts: (token: string) => request<Product[]>("/admin/products", { headers: authHeaders(token) }),
// //   createProduct: (token: string, formData: FormData) =>
// //     request<{ id: number; slug: string }>("/admin/products", { method: "POST", headers: authHeaders(token), body: formData }),
// //   updateProduct: (token: string, id: number, formData: FormData) =>
// //     request<{ id: number; slug: string }>(`/admin/products/${id}`, { method: "PUT", headers: authHeaders(token), body: formData }),
// //   deleteProduct: (token: string, id: number) =>
// //     request<void>(`/admin/products/${id}`, { method: "DELETE", headers: authHeaders(token) }),
// // };
...
//     items: { productSlug: string; size?: string; quantity: number }[];
//   }) =>
//     request<{ id: number; emailSent: boolean }>("/quotes", {
//       method: "POST",
