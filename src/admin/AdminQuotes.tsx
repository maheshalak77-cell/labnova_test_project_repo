...
//   const load = () => adminApi.getQuotes(token).then(setQuotes).catch((err) => setError(err instanceof Error ? err.message : "Could not load quotations."));
...
//     setQuotes((prev) => prev && prev.map((q) => (q.id === id ? { ...q, status: status as AdminQuote["status"] } : q)));
//     try {
...
//         <div><h1>Quotation Requests</h1><p>Every request submitted through the Contact Us page.</p></div>
...
//       {!filtered.length ? (
//         <div className="catalog-state">No quotation requests {statusFilter !== "all" ? `with status "${statusFilter}"` : "yet"}.</div>
//       ) : (
...
//                     <td><strong>{q.fullName}</strong><br/><small>{q.email}</small></td>
//                     <td>{q.items.length} product{q.items.length === 1 ? "" : "s"}</td>
//                     <td>{money(q.total)}</td>
//                     <td>{q.emailSent ? <span className="pill pill--ok">Sent</span> : <span className="pill pill--warn">Not sent</span>}</td>
//                     <td>{new Date(q.createdAt).toLocaleString("en-AU", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</td>
//                     <td>
//                       <select value={q.status} onChange={(e) => changeStatus(q.id, e.target.value)} className={`admin-select admin-select--status status-${q.status}`}>
//                         {STATUS_OPTIONS.map((s) => <option key={s} value={s}>{s[0].toUpperCase() + s.slice(1)}</option>)}
//                       </select>
//                     </td>
//                     <td><button className="admin-link" onClick={() => setExpanded(expanded === q.id ? null : q.id)}>{expanded === q.id ? "Hide" : "View"}</button></td>
//                   </tr>
//                   {expanded === q.id && (
//                     <tr className="admin-table__detail-row">
//                       <td colSpan={7}>
//                         <div className="quote-detail">
//                           {q.phone && <p><strong>Phone:</strong> {q.phone}</p>}
//                           {q.message && <p><strong>Message:</strong> {q.message}</p>}
//                           <table className="admin-subtable">
//                             <thead><tr><th>Product</th><th>Size</th><th>Qty</th><th>Unit price</th><th>Line total</th></tr></thead>
//                             <tbody>
//                               {q.items.map((item, i) => (
//                                 <tr key={i}>
//                                   <td>{item.productName}{item.sku ? <small> · {item.sku}</small> : null}</td>
//                                   <td>{item.size || "—"}</td>
//                                   <td>{item.quantity}</td>
